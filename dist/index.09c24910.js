...
$("#submit-btn").click(function() {
    let errorMessage = "", fieldsMissing = "";
    if ($("#name").val() == "") fieldsMissing += "<li>Name</li>";
    if ($("#email").val() == "") fieldsMissing += "<li>Email Address</li>";
    if ($("#phone").val() == "") fieldsMissing += "<li>Phone Number</li>";
    if ($("#password").val() == "") fieldsMissing += "<li>Password</li>";
    if ($("#password-confirm").val() == "") fieldsMissing += "<li>Password Confirmation</li>";
    if (fieldsMissing != "") errorMessage += "<p>The following field(s) are missing: </p>" + fieldsMissing;
    if (isEmail($("#email").val()) == false) errorMessage += "<p>Please enter a valid email address</p>"; // THE += OPERATOR APPENDS THE ASSIGNED STRING VALUE TO THE END OF THE CONTENTS OF errorMessage
    if ($.isNumeric($("#phone").val()) == false) errorMessage += "<p>Please enter a valid phone number</p>";
    if ($("#password").val() != $("#password-confirm").val()) errorMessage += "<p>Please check the passwords match</p>";
    if (errorMessage != "") $("#error-message").html(errorMessage);
